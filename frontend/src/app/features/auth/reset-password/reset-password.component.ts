...
export class ResetPasswordComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (!this.token) {
      this.erro = 'Link de redefinição inválido ou incompleto.';
    }
  }
  ...
  onSubmit() {
    ...
    this.auth.resetPassword(this.token, this.novaSenha).subscribe({
      next: (res) => {
        this.mensagem = res?.mensagem ?? 'Senha redefinida com sucesso!';
        this.sucesso = true;
        this.loading = false;
      },
      error: (err) => {
        this.erro = err.error?.erro ?? 'Não foi possível redefinir a senha. O link pode ter expirado.';
        this.loading = false;
      },
    });
  }
